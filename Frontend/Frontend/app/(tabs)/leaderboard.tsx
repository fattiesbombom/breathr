...
export default function LeaderboardScreen() {
  ...
  const loadLeaderboard = async () => {
    try {
      ...
      const friendsList = friendsResponse.ok ? (friendsData.friends || []) : [];
      const currentUser = meResponse.ok ? meData.user : null;
      ...
      if (currentUser) {
        ...
        const allUsers = [
          ...friendsList.filter((f: FriendUser) => f.id !== currentUser.id),
          {
            ...
            guardian_username: currentUser.guardian_username,
          }
        ];
        ...
        // If we can't get current user, just show friends
        const sortedFriends = friendsList.sort((a: FriendUser, b: FriendUser) => (b.death_count || 0) - (a.death_count || 0));
  ...
  const handleGrieve = async (targetUser: FriendUser) => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        console.error('Failed to send grieve notification:', data.error);
      }
    } catch (error) {
      console.error('Error sending grieve notification:', error);
    }
  };
    ...
    return (
      <View key={user.id} style={[styles.tombstoneContainer, { width: rank === 1 ? '34%' : '30%' }]}>
        <View style={[
          ...
          {!isCurrentUser && user.guardian_username && (
            <TouchableOpacity 
